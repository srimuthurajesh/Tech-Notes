...
| Question                                      | Answer                                                                                     |
|-----------------------------------------------|-------------------------------------------------------------------------------------------|
| What is JDK, JRE, JVM?                        | **JDK**: Java Development Kit; **JRE**: Java Runtime Environment; **JVM**: Java Virtual Machine. |
| What is Bytecode?                             | Bytecode is platform-independent, intermediate code generated by the Java compiler.       |
| What are types of comments in Java?           | Single-line (`//`), Multi-line (`/* */`), Documentation (`/** */`).                       |
| What is Identifier?                           | A name for variables, methods, classes, or other entities.                                |
| What is Literal?                              | Fixed values like `5`, `true`, `"Hello"`.                                                 |
| What is an Expression?                        | A combination of variables, operators, and values that evaluates to a single value.       |
| What are Keywords in Java?                    | Reserved words like `int`, `class`, `static`.                                             |
| Write Java input/output syntax.               | Input: `Scanner sc = new Scanner(System.in);` Output: `System.out.println("Hello");`      |
| What are Datatypes and its types?             | Define data types: **Primitive** (e.g., `int`, `float`), **Non-Primitive** (e.g., `String`, arrays). |
| Java is not 100% OOPs language, why?          | It supports primitive data types, which are not objects.                                  |
...
| What is String?                               | A sequence of characters. **Methods**: `length()`, `substring()`, `indexOf()`, `toLowerCase()`. |
